...
1. **Scan the project:** Run `npx @hopsule/vibechecker .` (or `npx @hopsule/vibechecker <path>`). If the project is Next.js, use `--next` for the full rule set.
...
3. **Apply fixes when safe:** If the user agrees, run `npx @hopsule/vibechecker . --fix` to apply autofixes (e.g. add `"use client"`, comment out `console.log`). The CLI will prompt if there are uncommitted changes.
