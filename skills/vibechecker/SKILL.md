...
3. **Apply fixes when safe:** If the user agrees, run `npx vibecode-check . --fix` to apply autofixes (e.g. add `"use client"`, comment out `console.log`). The CLI will prompt if there are uncommitted changes.
